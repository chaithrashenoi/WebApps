...
<nav id="tabs_nav">
  <ul>
		...
		<li class="active"><a href="/jadrn026/servlet/proj3/ShowCart">Show Cart(<%= request.getParameter("count") %>)</a></li>
  </ul>
</nav>
...
<th id="thead" style="width: 250px;" ><b>Quantity</b></th>	
<th id="thead"><b>UnitPrice</b></th>	
<th id="thead"><b>TotalPrice</b></th>	
<th id="thead"><b>Remove</b></th>	
<%
...
<tr width="700px" height="120px">
	  ...
	  <%= request.getAttribute("vendor"+i) %> <%= request.getAttribute("man_id"+i) %></a></td>
		<td id="tdata"><input type="number" class="quantity_cart" id ="quantity_cart<%=Sku%>" size="3" value="<%= request.getAttribute("quantity"+i) %>"  /></br></br>
		<input type="button" class="update_cart" title="Update quantity" id ="update_cart<%=Sku%>" size="3" value="Update"  />
		...
		<td id="tdata"><input type="button" class="remove_cart" title="Remove product from cart" id ="remove_cart<%=Sku%>" size="3" /></td>
</tr >
...
<% } %>
	...
	<div>
		<form method="post"
					name="shopcheck">
		<input   type="button" name="shopping" value="Continue Shopping" class="high" style="background-color:#424242; color: #D0ECF9;
		font-size: 15px; border-radius: 10px; "/>
		
		<input type="button" name="checkout" value="Check Out" class="high" style="background-color:#424242; color: #D0ECF9; font-size: 15px; border-radius: 10px;"/>
		</form><br>
	</div>
</div>
